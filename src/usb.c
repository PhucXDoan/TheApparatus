	...
	if (UDINT & (1 << SOFI)) // Start-of-Frame.
	{
		...
		switch (_usb_ms_state)
		{
				...
				if (UEINTX & (1 << RXOUTI))
				{
					...
					if (UEBCX == sizeof(command))
					{
						...
						)
						{
							...
							switch (command.CBWCB[0])
							{
								...
								case USBMSSCSIOpcode_inquiry: // See: Source(13) @ Section(3.6.1) @ Page(92).
								{
									...
									)
									{
										...
										_usb_ms_internal_info_data = USB_MS_SCSI_INQUIRY_DATA;
										_usb_ms_internal_info_size = sizeof(USB_MS_SCSI_INQUIRY_DATA);
								...
								case USBMSSCSIOpcode_mode_sense: // TODO Absolute fkin' mystery. See: Source(13) @ Section(3.11) @ Page(111).
								{
									...
									{
										_usb_ms_internal_info_data = USB_MS_SCSI_MODE_SENSE;
										_usb_ms_internal_info_size = sizeof(USB_MS_SCSI_MODE_SENSE);
								...
								case USBMSSCSIOpcode_read_format_capacities: // See: Source(14) @ Table(701) @ AbsPage(1).
								{
									...
									{
										_usb_ms_internal_info_data = USB_MS_SCSI_READ_FORMAT_CAPACITIES_DATA;
										_usb_ms_internal_info_size = sizeof(USB_MS_SCSI_READ_FORMAT_CAPACITIES_DATA);
								...
								case USBMSSCSIOpcode_read_capacity: // See: Source(13) @ Section(3.22) @ Page(155).
								{
									...
									{
										_usb_ms_internal_info_data = USB_MS_SCSI_READ_CAPACITY_DATA;
										_usb_ms_internal_info_size = sizeof(USB_MS_SCSI_READ_CAPACITY_DATA);
								...
								{
									u32 logical_block_count = ((command.CBWCB[7] << 8) | command.CBWCB[8]);
									if
									(
										command.bmCBWFlags &&
										command.bCBWCBLength == 10 &&
										command.dCBWDataTransferLength == logical_block_count * FAT32_SECTOR_SIZE &&
										command.CBWCB[1] == 0 && // Some random bits??
										command.CBWCB[6] == 0 && // "GROUP NUMBER"
										command.CBWCB[9] == 0 // "CONTROL", just in case it's not zero.
									)
									{
										_usb_ms_logical_block_address =
											(((u32) command.CBWCB[2]) << 24) |
											(((u32) command.CBWCB[3]) << 16) |
											(((u32) command.CBWCB[4]) <<  8) |
											(((u32) command.CBWCB[5]) <<  0);
										_usb_ms_logical_block_remaining      = logical_block_count;
										_usb_ms_logical_block_fragment_index = 0;
									}
									else
									{
										stall = true;
							...
							else if (_usb_ms_internal_info_size)
							{
								...
								_usb_ms_status =
									(struct USBMSCommandStatusWrapper)
									{
										...
										.dCSWDataResidue = command.dCBWDataTransferLength - _usb_ms_internal_info_size,
										.bCSWStatus      = 0x00,
									};
							}
			...
			case USBMSState_sending_data:
			{
				...
				if (UEINTX & (1 << TXINI))
				{
					if (_usb_ms_internal_info_size)
					{
						for (u8 i = 0; i < _usb_ms_internal_info_size; i += 1)
						{
							UEDATX = ((u8*) &_usb_ms_internal_info_data)[i];
						}
						_usb_ms_internal_info_size = 0;
					}
					else if (_usb_ms_logical_block_remaining)
					{
						switch (_usb_ms_logical_block_address)
						{
							case 0:
							{
								...
								{
									UEDATX = pgm_read_byte(&((u8*) &FAT32_MASTER_BOOT_RECORD)[_usb_ms_logical_block_fragment_index * USB_ENDPOINT_MS_IN_SIZE + i]);
								}
							} break;

							case FAT32_PARTITION_SECTOR_ADDRESS:
							case FAT32_PARTITION_SECTOR_ADDRESS + FAT32_BACKUP_BOOT_SECTOR_OFFSET:
							{
								for (u8 i = 0; i < USB_ENDPOINT_MS_IN_SIZE; i += 1)
								{
									UEDATX = pgm_read_byte(&((u8*) &FAT32_BOOT_SECTOR)[_usb_ms_logical_block_fragment_index * USB_ENDPOINT_MS_IN_SIZE + i]);
								}
							} break;

							case FAT32_PARTITION_SECTOR_ADDRESS + FAT32_FILE_STRUCTURE_INFO_SECTOR_OFFSET:
							case FAT32_PARTITION_SECTOR_ADDRESS + FAT32_FILE_STRUCTURE_INFO_SECTOR_OFFSET + FAT32_BACKUP_BOOT_SECTOR_OFFSET:
							{
								for (u8 i = 0; i < USB_ENDPOINT_MS_IN_SIZE; i += 1)
								{
									UEDATX = pgm_read_byte(&((u8*) &FAT32_FILE_STRUCTURE_INFO)[_usb_ms_logical_block_fragment_index * USB_ENDPOINT_MS_IN_SIZE + i]);
								}
							}

							case FAT32_PARTITION_SECTOR_ADDRESS + FAT32_RESERVED_SECTOR_COUNT:
							{
								for (u8 i = 0; i < USB_ENDPOINT_MS_IN_SIZE; i += 1)
								{
									UEDATX = pgm_read_byte(&((u8*) &FAT32_TABLE)[_usb_ms_logical_block_fragment_index * USB_ENDPOINT_MS_IN_SIZE + i]);
								}
							} break;

							case FAT32_PARTITION_SECTOR_ADDRESS + FAT32_RESERVED_SECTOR_COUNT + FAT32_TABLE_SECTOR_COUNT:
							{
								for (u8 i = 0; i < USB_ENDPOINT_MS_IN_SIZE; i += 1)
								{
									UEDATX = pgm_read_byte(&((u8*) &FAT32_ROOT_DIR_ENTRIES)[_usb_ms_logical_block_fragment_index * USB_ENDPOINT_MS_IN_SIZE + i]);
								}
							} break;

							case FAT32_PARTITION_SECTOR_ADDRESS + FAT32_RESERVED_SECTOR_COUNT + FAT32_TABLE_SECTOR_COUNT + (3 - FAT32_ROOT_CLUSTER) * FAT32_SECTORS_PER_CLUSTER: // TODO Coupled data.
							{
								for (u8 i = 0; i < USB_ENDPOINT_MS_IN_SIZE; i += 1)
								{
									UEDATX = pgm_read_byte(&((u8*) &FAT32_SYSTEM_VOLUME_INFO_DIR_ENTRIES)[_usb_ms_logical_block_fragment_index * USB_ENDPOINT_MS_IN_SIZE + i]);
								}
							} break;
						...
						if (_usb_ms_logical_block_fragment_index == FAT32_SECTOR_SIZE / USB_ENDPOINT_MS_IN_SIZE)
						{
							_usb_ms_logical_block_address        += 1;
							_usb_ms_logical_block_remaining      -= 1;
							_usb_ms_logical_block_fragment_index  = 0;
						}
					}

					if (!_usb_ms_logical_block_remaining)
					{
						_usb_ms_state = USBMSState_ready_for_status;
					}
