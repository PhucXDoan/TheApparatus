						...
						)
						{
							if (subword_bits & (1 << 15))
							{
								...
								switch (diplomat_packet.wordgame)
								{
									...
									// This algorithm for reproducing words in WordBites is suboptimal, but it performs well enough in practice
									// that I believe it's sufficient. Since words can be created horizontally or vertically, we first see if
									// the word is reproducible horizontally, and if not, we just simply transpose the entire game and try again.
									// This allows for us to essentially handle both cases of horizontal and vertical words. There's also the
									// issue of how exactly do we move the pieces in such a way that it recreates the word optimally, i.e. in the
									// shortest amount of time. Perhaps this problem is best left for AI to figure out, but what I essentially did
									// was just try to make the word at every single slot of the board, both vertically and horizontally, and keep
									// track of the best sequence of commands.
...
/* [Overview].
	This is the Nerd; it handles all computations relating to reproducing words for the wordgames.
	It depends on an SD card loaded with a binary file generated by Microservices.exe that contains
	a compressed stream of words from all languages that Game Pigeon implements. Nerd waits for it
	to receive the packet from the Diplomat that details what wordgame it should be processing for
	and the letters within it. When the Nerd finds that a word is reproducible, it sends mouse
	commands to the Diplomat instructing on how to recreate and submit that word. The Diplomat is
	heavily bottlenecked by mouse commands (mainly due to Apple's buggy UI), so the Nerd spends
	most of its time waiting for the Diplomat to be unbusy.
*/
