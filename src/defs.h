...
enum PinState
{
	...
	#define PIN_XMDT(X) /* ATmega32U4. */ \
		...
		X(18, F, 1) X(19, F, 0) \
		X(50, B, 3) X(51, B, 2) X(52, B, 1) X(53, B, 0) /* These aren't on Arduino boards; just copied from the Mega2560. */
	...
	*/
	...
	#define LCD_REGISTER_SELECT_PIN 48
	#define LCD_ENABLING_PIN        46
	#define LCD_D4_PIN              45
	#define LCD_D5_PIN              44
	#define LCD_D6_PIN              43
	#define LCD_D7_PIN              42
	#define LCD_DIMS_X              20
	#define LCD_DIMS_Y              4
	static char lcd_backbuffer[LCD_DIMS_Y][LCD_DIMS_X] = {0};
	static i8   lcd_cursor_pos_x                       = 0;
	static i8   lcd_cursor_pos_y                       = 0;
#endif
...
#if PROGRAM_DIPLOMAT
static volatile struct SPIPacket spi_packet_buffer[16] = {0};
static_assert(sizeof(spi_packet_buffer) <= 1024);
#endif

#if PROGRAM_NERD
static volatile struct SPIPacket spi_packet_buffer[128] = {0};
static_assert(sizeof(spi_packet_buffer) <= 6000);
#endif

static_assert(countof(spi_packet_buffer) && !(countof(spi_packet_buffer) & (countof(spi_packet_buffer) - 1)));

static volatile u8 spi_packet_writer    = 0;
static volatile u8 spi_packet_reader    = 0;
#define spi_packet_writer_masked(OFFSET) ((spi_packet_writer + OFFSET) & (countof(spi_packet_buffer) - 1))
#define spi_packet_reader_masked(OFFSET) ((spi_packet_reader + OFFSET) & (countof(spi_packet_buffer) - 1))
