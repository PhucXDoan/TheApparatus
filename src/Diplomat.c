...
main(void)
{
	...
	while (true)
	{
		...
		if
		(
			(first_displayed_map + LCD_DIM_Y) % WordGameMap_COUNT == selected_map ||
			(selected_map + 1               ) % WordGameMap_COUNT == first_displayed_map
		)
		{
		...
		for (u8 row = 0; row < LCD_DIM_Y; row += 1)
		{
			...
			lcd_pgm_cstr(WORDGAME_MAP_PRINT_NAME_CSTR[map]);
			lcd_char('\n');
		}
